...
public final class ShardingTablesOnlyForPStatementWihGroupByTest extends AbstractShardingTablesOnlyDBUnitTest {
...
//        assertDataSet("integrate/dataset/tbl/expect/select_group_by/SelectSum.xml", shardingDataSource.getConnection(), "t_order", sql);
    ...
    public void assertSelectOrderByDesc() throws SQLException, DatabaseUnitException {
        String sql = "SELECT SUM(order_id) AS `orders_sum`, `user_id` FROM `t_order` GROUP BY `user_id` ORDER BY user_id DESC";
        try (
                ShardingConnection conn = shardingDataSource.getConnection();
                Statement stmt = conn.createStatement();
                ResultSet rs = stmt.executeQuery(sql)
